...
export default function App() {
  ...
  function onRenameEmpresa(which, novoNome) {
    setData(prev => {
      const oldVal = which === "A" ? prev.meta.empresaA : prev.meta.empresaB;
      ...
      return {
        ...prev,
        meta: { ...prev.meta, [which === "A" ? "empresaA" : "empresaB"]: novoNome },
  ...
  function limparTudo() {
    if (!confirm("Isso vai apagar todos os dados salvos e recomeçar do zero. Confirma?")) return;
    limpar();
  }
  ...
  function renderPanel() {
    switch (tab) {
      ...
      case "alunos":
        return <AlunosPanel data={data} onFieldChange={onFieldChange} />;
      case "escalacao":
        return <EscalacaoPanel data={data} />;
      case "sm":
        return <ScrumMasterPanel rows={data.sm} onFieldChange={onFieldChange} />;
      case "owner":
        return <OwnerPanel rows={data.owner} onFieldChange={onFieldChange} />;
      case "po":
        return <ProductOwnerPanel rows={data.po} onFieldChange={onFieldChange} />;
      case "dev":
        return <DevPanel rows={data.dev} onFieldChange={onFieldChange} />;
      case "buyerProf":
        return <BuyerProfilePanel rows={data.buyerProf} onFieldChange={onFieldChange} />;
      case "buyerProduct":
        return <BuyerProductPanel rows={data.buyerProduct} onFieldChange={onFieldChange} />;
      case "corrupsab":
        return <CorrupcaoSabotagemPanel data={data} onFieldChange={onFieldChange} />;
      case "result":
    ...
    <div>
      <div className="topbar">
        <div>
          ...
          <div className="sub">
            Empresa A: {data.meta.empresaA} · Empresa B: {data.meta.empresaB}
            {salvoEm && <> · salvo às {salvoEm.toLocaleTimeString()}</>}
          </div>
        </div>
        <div className="topbar-actions">
          <button className="btn btn-save" onClick={salvarAgora}>💾 Salvar agora</button>
          <button className="btn btn-reset" onClick={limparTudo}>Limpar tudo</button>
        </div>
      </div>
      ...
      <div className="wrap">
        ...
        <div className="footer-note">
          Os dados são salvos automaticamente neste navegador a cada alteração.
        </div>
      </div>
    </div>
  );
